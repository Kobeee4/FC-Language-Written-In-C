...
2. Parser (src/parser.c / src/parser.h)
   ...
   - Implements expression parsing, statements, function/class definitions, control flow, and error recovery.
...
5. Native modules (src/modules.c and related C files)
   ...
   - Modules expose a simple C API to scripts via dicts mapping names to native functions.
...
- bin/: produced by the top-level Makefile (produces bin/fcl for quick testing)
- docs/: this folder (syntax, architecture, neural runtime notes, roadmap)
- top-level Makefile: convenience wrapper I added to build and place an executable at bin/fcl

Notes on extendability
- Adding a native module:
  1. Implement C functions following `Value f(Interp*, Value, int, Value*)` signature
  2. Build a dict of functions and register with `register_my_mod(Interp* I)`
  3. Add the file to `SRCS` in src/Makefile and call `make`
- The Module guide in `src/Module guide.txt` explains this step-by-step.
...
- Memory ownership uses simple helpers and manual release; interpreter manages lifetime for returned values

Testing and CI
- Currently manual. I can add a GitHub Actions workflow to run `make` on pushes and PRs.

If you want, I can expand this architecture doc with sequence diagrams, memory ownership notes, or a file-level map showing where core systems live.